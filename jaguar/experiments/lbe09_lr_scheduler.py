...
EXPERIMENT_CONFIGS = [
    {
        ...
        "LR_scheduler": "None",  # Baseline: constant learning rate
    },
    {
        "epochs": 100,
        "batch_size": 64,
    ...
    },
    {
        ...
        "LR_gamma": 0.95,  # Decay factor per epoch
    },
]
...
# Store results across seeds for each scheduler
scheduler_results = {}  # {scheduler_name: {seed: {mAP, diverged, etc.}}}

for EXPERIMENT_CONFIG in EXPERIMENT_CONFIGS:
    scheduler_name = EXPERIMENT_CONFIG["LR_scheduler"]
    ...
    for random_seed in BASE_CONFIG["random_seeds"]:
        ...
        run_name = f"{GROUP}-{scheduler_name}-seed{random_seed}"
        ...
        lr_scheduler = None
        if EXPERIMENT_CONFIG["LR_scheduler"] == "ReduceLROnPlateau":
            lr_scheduler = ReduceLROnPlateau(optimizer, mode="min", factor=EXPERIMENT_CONFIG["LR_factor"], patience=EXPERIMENT_CONFIG["LR_patience"])
        elif EXPERIMENT_CONFIG["LR_scheduler"] == "StepLR":
            lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=EXPERIMENT_CONFIG["LR_step_size"], gamma=EXPERIMENT_CONFIG["LR_gamma"])
        elif EXPERIMENT_CONFIG["LR_scheduler"] == "CosineAnnealingLR":
            lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=EXPERIMENT_CONFIG["epochs"])
        elif EXPERIMENT_CONFIG["LR_scheduler"] == "ExponentialLR":
            lr_scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer, gamma=EXPERIMENT_CONFIG["LR_gamma"])
        elif EXPERIMENT_CONFIG["LR_scheduler"] == "OneCycleLR":
        ...
        for epoch in range(EXPERIMENT_CONFIG["epochs"]):
            ...
            start = time.time()
            # OneCycleLR needs to step after each batch, so pass it to train_epoch
            if EXPERIMENT_CONFIG["LR_scheduler"] == "OneCycleLR":
                train_loss = train_epoch(model, train_dataloader, optimizer, device, lr_scheduler=lr_scheduler)
            else:
            ...
            if lr_scheduler is not None:
                ...
                elif EXPERIMENT_CONFIG["LR_scheduler"] in ["StepLR", "CosineAnnealingLR", "ExponentialLR"]:
                    lr_scheduler.step()
        ...
        scheduler_results[scheduler_name][random_seed] = {
...
# Aggregate results across seeds for each scheduler
print("\n" + "=" * 80)
print("SCHEDULER COMPARISON RESULTS")
print("=" * 80)
for scheduler_name, seed_results in scheduler_results.items():
