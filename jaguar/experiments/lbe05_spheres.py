...
from jaguar.components import DINOv3, VielleichtguarModel, EmbeddingProjection, HypersphericalProjection, HyperbolicProjection
from jaguar.criteria import ArcFaceCriterion, HyperbolicArcFaceCriterion
...
GROUP = "15_hyperbolicVShyperspherical"
...
EXPERIMENT_CONFIGS = [
    {
        ...
        "embedding_space": "euclidean",  # baseline for reference
    },
    {
        "epochs": 100,
        "batch_size": 64,
        ...
        "curvature": 1.0,       # only used for hyperbolic
        "max_norm": 0.5,        # Poincaré ball clipping radius
    },
    {
        "epochs": 100,
        "batch_size": 64,
        ...
        "embedding_space": "hyperbolic",
        "curvature": 1.0,       # only used for hyperbolic
        "max_norm": 0.6,        # Poincaré ball clipping radius
    },
    {
        "epochs": 100,
        "batch_size": 64,
        ...
        "train_backbone": False,
        "embedding_space": "hyperbolic",
        "curvature": 2.0,       # only used for hyperbolic
        "max_norm": 0.7,        # Poincaré ball clipping radius
    },
    {
        "epochs": 100,
        "batch_size": 64,
        ...
        "patience": 10,
        "train_backbone": False,
        "embedding_space": "hyperspherical",
    },
]
...
for EXPERIMENT_CONFIG in EXPERIMENT_CONFIGS:
    embedding_space = EXPERIMENT_CONFIG["embedding_space"]
    RUN_NAME = f"{GROUP}-{embedding_space}"
    ...
    if embedding_space == "euclidean":
    ...
    elif embedding_space == "hyperspherical":
        projection = HypersphericalProjection(
            input_dim=backbone.out_dim(),
            hidden_dim=EXPERIMENT_CONFIG["hidden_dim"],
            output_dim=EXPERIMENT_CONFIG["output_dim"],
            dropout=EXPERIMENT_CONFIG["dropout"],
    ...
    elif embedding_space == "hyperbolic":
        projection = HyperbolicProjection(
            input_dim=backbone.out_dim(),
            hidden_dim=EXPERIMENT_CONFIG["hidden_dim"],
            output_dim=EXPERIMENT_CONFIG["output_dim"],
            dropout=EXPERIMENT_CONFIG["dropout"],
            curvature=EXPERIMENT_CONFIG["curvature"],
            max_norm=EXPERIMENT_CONFIG["max_norm"],
        )
        criterion = HyperbolicArcFaceCriterion(
            embedding_dim=EXPERIMENT_CONFIG["output_dim"],
            num_classes=num_classes,
            margin=EXPERIMENT_CONFIG["arcface_margin"],
            scale=EXPERIMENT_CONFIG["arcface_scale"],
        ...
        raise ValueError(f"Unknown embedding_space: {embedding_space!r}. Choose 'hyperspherical' or 'hyperbolic'.")

    model = VielleichtguarModel(
        backbone=backbone,
        layers=nn.Sequential(projection),
        criterion=criterion,
    ).to(device)
    ...
    for epoch in range(EXPERIMENT_CONFIG["epochs"]):
        ...
        print(
            f"[{embedding_space}] epoch: {epoch+1:>2}/{EXPERIMENT_CONFIG['epochs']} | ",
